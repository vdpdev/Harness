...
import { LOCK_PATH, LOCK_RETRY_MS, LOCK_TIMEOUT_MS } from '../tickets.js';
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        if (Date.now() >= deadline) {
          throw new Error(
            `tickets.ts: could not acquire lock ${LOCK_PATH} within ${LOCK_TIMEOUT_MS}ms ` +
              `(concurrent writes would corrupt tickets.json)`,
          );
        }
        try {
